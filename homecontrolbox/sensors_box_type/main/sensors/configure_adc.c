...
void init_adc1(){
    ...
    ret = adc_oneshot_config_channel(adc_handle, ADC_CHANNEL_BAT, &chan_config);

    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to configure ADC channel %d: %s", ADC_CHANNEL_BAT, esp_err_to_name(ret));
    }
}
